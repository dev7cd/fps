# SPDX-FileCopyrightText: 2026 Devine Ngouloubi <exauce-devine.ngouloubi@unicaen.fr>
# SPDX-License-Identifier: AGPL-3.0-or-later

import matplotlib.pyplot as plt
import numpy as np
import logging
from typing import List, Dict, Optional
from mpl_toolkits.mplot3d import Axes3D

# Avoid the interactive backend when launched headless (server)
try:
    import matplotlib
    matplotlib.use('Agg') # Force non-interactive backend for file saving
except:
    pass

## @var logger
#  @brief Logger for the plotter module.
logger = logging.getLogger(__name__)


class Visualizer:
    """!
    @brief Scientific visualization tools for fiber microstructures.
    @details Provides methods for 3D RVE rendering, orientation tensor analysis,
    and statistical plotting.
    """

    @staticmethod
    def plot_rve(fibers: List, box_dims, filename: Optional[str] = None) -> None:
        """!
        @brief Simple 3D geometric visualization of fiber skeletons.
        @param fibers List List of Fiber objects to plot.
        @param box_dims tuple (Lx, Ly, Lz) representing domain dimensions.
        @param filename Optional[str] Optional path to save the generated image.
        @return None
        """
        fig = plt.figure(figsize=(10, 10))
        ax = fig.add_subplot(111, projection='3d')

        # Box
        lx, ly, lz = box_dims
        # Box edges
        lines = [
            ([0,lx], [0,0], [0,0]), ([0,lx], [ly,ly], [0,0]),
            ([0,lx], [0,0], [lz,lz]), ([0,lx], [ly,ly], [lz,lz]),
            ([0,0], [0,ly], [0,0]), ([lx,lx], [0,ly], [0,0]),
            ([0,0], [0,ly], [lz,lz]), ([lx,lx], [0,ly], [lz,lz]),
            ([0,0], [0,0], [0,lz]), ([lx,lx], [0,0], [0,lz]),
            ([0,0], [ly,ly], [0,lz]), ([lx,lx], [ly,ly], [0,lz])
        ]
        for x,y,z in lines:
            ax.plot(x,y,z, 'k-', lw=0.5, alpha=0.3)

        # Fibers
        for f in fibers:
            pts = f.centerline
            if len(pts) < 2: continue

            style = '--' if f.is_ghost else '-'
            alpha = 0.4 if f.is_ghost else 1.0
            color = 'gray' if f.is_ghost else None

            # Orientation colour (if not a ghost)
            if not f.is_ghost:
                vec = pts[-1] - pts[0]
                n = np.linalg.norm(vec)
                color = np.abs(vec/n) if n>0 else (0,0,1)

            ax.plot(pts[:,0], pts[:,1], pts[:,2],
                   linestyle=style, color=color, alpha=alpha, lw=1.5)

        ax.set_aspect('equal')
        ax.set_axis_off()

        if filename:
            plt.savefig(filename, dpi=150, bbox_inches='tight')
            logger.info(f"3D render saved: {filename}")
        plt.close(fig)

    @staticmethod
    def plot_analysis_report(stats_results: Dict, output_prefix: str) -> None:
        """!
        @brief Generates a scientific analysis report with orientation plots.
        @details Visualizes AD-PCA orientation tensors as heatmaps and 3D ellipsoids.
        @param stats_results Dict Dictionary containing AD-PCA results and Herman's factors.
        @param output_prefix str String prefix for the output filenames.
        @return None
        """
        if not stats_results: return

        logger.info(f"Generating analysis plots to {output_prefix}_*.png ...")

        # Retrieve the raw data temporarily stored in the dictionary
        # (Requires RVEAnalyzer to pass the raw lists, not just the means.)
        # If they are not in stats_results (JSON-ready), we cannot plot.
        # Ideally, RVEAnalyzer should return the arrays 'taus', 'u_vecs', 'v_vecs'.

        # To keep it simple here, we visualise what we have: the global tensor.

        # --- PLOT 1: orientation tensors (heatmap) ---
        fig, axes = plt.subplots(1, 2, figsize=(10, 4))

        # Axial Tensor
        ax_mat = np.array(stats_results['ad_pca']['tensors']['A_axial'])
        im1 = axes[0].imshow(ax_mat, cmap='viridis', vmin=0, vmax=1)
        axes[0].set_title(f"Axial tensor ($f_a={stats_results['ad_pca']['anisotropy_spectrum']['f_axial']:.2f}$)")
        for (i, j), z in np.ndenumerate(ax_mat):
            axes[0].text(j, i, f'{z:.2f}', ha='center', va='center', color='w' if z<0.7 else 'k')

        # Planar Tensor
        pl_mat = np.array(stats_results['ad_pca']['tensors']['A_planar'])
        im2 = axes[1].imshow(pl_mat, cmap='plasma', vmin=0, vmax=1)
        axes[1].set_title(f"Planar tensor ($f_p={stats_results['ad_pca']['anisotropy_spectrum']['f_planar']:.2f}$)")
        for (i, j), z in np.ndenumerate(pl_mat):
            axes[1].text(j, i, f'{z:.2f}', ha='center', va='center', color='w' if z<0.7 else 'k')

        plt.colorbar(im1, ax=axes[0])
        plt.colorbar(im2, ax=axes[1])
        plt.tight_layout()
        plt.savefig(f"{output_prefix}_tensors.png")
        plt.close()

        # --- PLOT 2: orientation ellipsoid (3D visualisation of the tensor) ---
        # Visualise the ellipsoid representing A_axial
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')

        # Eigen decomposition
        evals, evecs = np.linalg.eigh(ax_mat)

        # Deformed unit sphere
        u = np.linspace(0, 2 * np.pi, 30)
        v = np.linspace(0, np.pi, 30)
        x = np.outer(np.cos(u), np.sin(v))
        y = np.outer(np.sin(u), np.sin(v))
        z = np.outer(np.ones(np.size(u)), np.cos(v))

        # Transformation
        for i in range(len(x)):
            for j in range(len(x)):
                [x[i,j], y[i,j], z[i,j]] = np.dot(evecs, [x[i,j]*evals[0], y[i,j]*evals[1], z[i,j]*evals[2]])

        # Plot
        ax.plot_surface(x, y, z, color='b', alpha=0.3, edgecolor='k')

        # Principal axes
        origin = np.zeros(3)
        for i in range(3):
            vec = evecs[:, i] * evals[i] * 1.2
            ax.quiver(*origin, *vec, color=['r','g','b'][i], lw=2)

        ax.set_title("Axial orientation ellipsoid")
        ax.set_aspect('equal')
        plt.savefig(f"{output_prefix}_ellipsoid.png")
        plt.close()


def plot_rve(fibers: List, dims: tuple, filename: Optional[str] = None) -> None:
    """!
    @brief Functional wrapper for Visualizer.plot_rve.
    @param fibers List List of fiber objects.
    @param dims tuple Domain dimensions (Lx, Ly, Lz).
    @param filename Optional[str] Optional path to save the generated image.
    @return None
    """
    Visualizer.plot_rve(fibers, dims, filename)


def plot_analysis(stats: Dict, prefix: str) -> None:
    """!
    @brief Functional wrapper for Visualizer.plot_analysis_report.
    @param stats Dict Statistics dictionary containing AD-PCA results.
    @param prefix str String prefix for the output filenames.
    @return None
    """
    Visualizer.plot_analysis_report(stats, prefix)
