...
class TestContainsPoint:
    ...
    def test_center_is_inside(self):
        ...
        assert v.contains_point(np.array([1.0, 1.0, 1.0])) is True

    def test_point_inside(self):
        v = Void(id=1, center=np.array([0.0, 0.0, 0.0]), radius=1.0)
        assert v.contains_point(np.array([0.5, 0.0, 0.0])) is True

    def test_point_on_surface(self):
        v = Void(id=1, center=np.array([0.0, 0.0, 0.0]), radius=1.0)
        assert v.contains_point(np.array([1.0, 0.0, 0.0])) is True

    def test_point_outside(self):
        v = Void(id=1, center=np.array([0.0, 0.0, 0.0]), radius=1.0)
        assert v.contains_point(np.array([2.0, 0.0, 0.0])) is False

    def test_point_just_outside(self):
        v = Void(id=1, center=np.array([0.0, 0.0, 0.0]), radius=1.0)
        assert v.contains_point(np.array([1.001, 0.0, 0.0])) is False

    def test_point_diagonal(self):
        v = Void(id=1, center=np.array([0.0, 0.0, 0.0]), radius=1.0)
        # Distance = sqrt(3) ≈ 1.732 > 1.0
        assert v.contains_point(np.array([1.0, 1.0, 1.0])) is False

    def test_offset_center(self):
        v = Void(id=1, center=np.array([5.0, 5.0, 5.0]), radius=0.5)
        assert v.contains_point(np.array([5.2, 5.1, 5.0])) is True
        assert v.contains_point(np.array([0.0, 0.0, 0.0])) is False

    def test_zero_radius_only_center(self):
        v = Void(id=1, center=np.array([1.0, 2.0, 3.0]), radius=0.0)
        assert v.contains_point(np.array([1.0, 2.0, 3.0])) is True
        assert v.contains_point(np.array([1.0, 2.0, 3.001])) is False
